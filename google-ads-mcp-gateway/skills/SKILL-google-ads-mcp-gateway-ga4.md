...
Use this skill when the user asks about **Google Analytics 4 (GA4)**: traffic, sessions, users, events, conversions, funnels, realtime, dimensions, metrics, **properties**, or **accounts** — not Google Ads campaigns, budgets, or GAQL.

**Same gateway as Ads:** the custom MCP server exposes **4 GA4 tools** plus **10 Ads tools** (**14 total** on `/mcp`). Use this skill only for the **`ga4_*`** tools; for Ads reporting or mutations, use the **Google Ads (MCP Gateway)** skill.
...
| Discovery | `ga4_list_properties` | List **properties** the user can access; read `name` like `properties/123456789` to get numeric **property_id**. |
...
There are **no GA4 write/mutation tools** on this gateway — only read/reporting.
...
- `dimensions` — array of objects, one object per dimension, e.g. `[{"name": "date"}, {"name": "sessionSource"}]`.
- `metrics` — array of objects, e.g. `[{"name": "sessions"}, {"name": "totalUsers"}]`.
...
1. If the user did not give a **property ID**, call **`ga4_list_properties`** (and optionally **`ga4_list_accounts`** to help interpret account names in the UI).
...
- **GA4 traffic, properties, reports** → this skill and **`ga4_*`** tools only.
- **Google Ads spend, campaigns, keywords, pausing ads, budgets** → **Google Ads (MCP Gateway)** skill and **`ads_*`** tools (including the seven mutation tools when the server allows them).
...
**Connector:** Same custom MCP gateway **`/mcp`** as Google Ads when OAuth includes **both** Ads (`adwords`) and **Analytics** (`analytics.readonly`); the user sees one connection and **14 tools** (`ads_*` + `ga4_*`). If GA4 calls fail with 401/403, re-authorize so the token includes **`analytics.readonly`**.
